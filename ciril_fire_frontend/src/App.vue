<template>
  <div id="app" class="flex flex-col items-center justify-center min-h-screen bg-gray-100">
    ...
    <!-- Grille dynamique -->
    <div v-if="grid.length" class="grid gap-2" :style="{ gridTemplateColumns: `repeat(${grid[0].length}, 1fr)` }">
      <div
...
export default {
  ...
  data() {
    return {
      grid: [], // Grille initialisée depuis le back-end
    };
  },
  methods: {
    async fetchGrid() {
  ...
  mounted() {
    this.fetchGrid(); // Récupère la grille initiale au chargement
  },
};
</script>
